...
def render_score_table(holes, hole_targets):
    ...
    for h in holes:
        ...
        strategy_html = f"<span style='color:{color}; font-weight:700;'>{label}({target})</span>"
...
st.markdown("<h1 style='color:#e53e3e;'>⛳ AIキャディ</h1>", unsafe_allow_html=True)
...
# ---------- ラウンドスコア戦略（折りたたみ） ----------
with st.expander(f"🧠 {target_score}打で回るためのラウンド戦略", expanded=False):
...
plan_col1, plan_col2, plan_col3 = st.columns([4, 3, 2])

with plan_col1:
    st.markdown('<div style="font-size:14px; font-weight:700; color:#1a2e44; margin-top:14px; white-space:nowrap;">⛳ このホールは何打で上がる？</div>', unsafe_allow_html=True)

with plan_col2:
    st.markdown(
        f"""
        <div style='padding-top:8px;'>
          <span class='recommend-badge' style='background:{rec_color}22; color:{rec_color}; border:1.5px solid {rec_color}; font-size:12px;'>
...
with st.form("shot_form"):
    ...
    st.markdown('<div class="ui-label">🎯 ショットの結果を入力</div>', unsafe_allow_html=True)
