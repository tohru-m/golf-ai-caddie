...
st.markdown('<div style="font-size:22px; font-weight:900; color:#b91c1c; margin-top:14px; margin-bottom:6px;">🎤 タップして音声で結果入力</div>', unsafe_allow_html=True)
...
with st.expander("⛳ コース設定", expanded=False):
    ...
    for h in st.session_state.course.keys():
        ...
        with c1:
            st.markdown(f"<div style='font-size:24px; font-weight:700; color:#2563eb; padding-top:10px;'>{h}番ホール</div>", unsafe_allow_html=True)
        with c2:
